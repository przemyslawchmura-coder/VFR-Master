...
`data/source-acquisition-status.js` records source acquisition as independent gates: publication identified, content accessible, authenticity verified, relevant section inspected, and field evidence extracted. A later gate never implies an earlier one (and a publication with no extracted values is not service-manual evidence). `reports/deep-profile-pipeline-status.md` combines these states with field coverage, critical blockers, applicability blockers and next source classes. `eligible-for-human-profile-review` is explicit, deterministic and remains false until the conservative readiness policy is satisfied; it is not a verification or automatic-promotion status. Promotion remains: source review → conflict resolution → applicability review → schema conversion → validator → human review → registry addition.
