...
async function buildReport() {
  const boundedProfiles = registry.listProfiles().filter(item => item.profileId !== "honda.cbr500r.pc70.2024");
  const before = JSON.stringify({ registry: boundedProfiles, profile, source });
  const beforeProfiles = boundedProfiles;
