...
const snapshot = {
  ...
  research: { canonicalFieldCount: standard.FIELD_COUNT, serviceCoreFieldCount: 44, targetCount: serviceReport.targetVariants, serviceCoreEvidence: serviceReport.serviceCoreEvidence, serviceCoreSlots: serviceReport.serviceCoreSlots, vfr800: { verified: serviceReport.byTarget.find(item => item.catalogVariantKey === "honda.vfr800.rc46.vtec.gen1").post.serviceCore.evidenceFound, total: 44, uncertainRows: serviceReport.vfrProofSummary["SOURCE-IDENTITY-UNCERTAIN"] }, cbr500r: { verified: serviceReport.byTarget.find(item => item.catalogVariantKey === "honda.cbr500r.pc70").post.serviceCore.evidenceFound, total: 44 }, batchWave2: { targets: batch.targets.length, documents: batch.documents.length, evidenceRows: batch.evidence.length, netNewVerifiedSlots: 6 } },
