...
The VFR800 production profile's `61MCW07` service-manual citations were independently checked against the underlying page/section references in the document registry and verification audits. The research layer now records 37 independently verified Service Core fields for MY2002 RC46 VTEC (84%): oil, coolant, plugs, VTEC valve specifications and conditions, chain, brakes, tires, electrical, maintenance and routine torques. Each row retains its production citation ID and comparison (`MATCH-NORMALIZED`); production values and the registry remain read-only. VFR800 is therefore research-only `SERVICE-CORE-READY`, not automatically promoted.
