...
Identity-mapping infrastructure is now available for a future bounded FZ1 source-authentication wave; the FZ1 planning wave authenticated one narrowly scoped owner-manual route and retained the service-manual code as a partial-authentication lead. The bounded `2D1X` acquisition wave acquired one official PDF and preserved its hash/provenance without inspection, extraction or promotion. The bounded extraction plan now fixes six structural regions, explicit applicability/provenance gates and a 24-candidate ceiling. A generic extraction playbook now captures those process rules and is proven compatible with both the mature VFR fingerprint pattern and the FZ1 plan; this does not reopen the completed audited scope or authorize extraction beyond the exact next pilot task.
...
Objective: verify auth, persistence, RLS, deployment and operational error handling. Entry: selected test backend and deployment environment. Completed: repository-controlled ownership/RLS baseline applied and verified live; repository-side password recovery flow implemented with fail-closed callback handling; operator-supplied production recovery path verified end-to-end; deployment/recovery hardening now requires an explicit production callback URL, binds recovery UI to a Supabase recovery event/session identity, and documents the operator boundary; permanent owner-first Rider Service Core and Source Trust Model governance recorded; repository clean-baseline migration foundation added, normalized to valid ordered identifiers, and replayed twice successfully in local PostgreSQL 17. The Dashboard allow-list remains an external configuration boundary. Leaked-password protection is externally blocked because project `vfr-master` is on the Free plan and Supabase makes the setting available only on Pro plan and above. Tasks remaining: prepare authorized production history-only reconciliation, operational recovery checks, future lawful source-link presentation and Garage/Service maintenance-due integration. Exit: evidence-backed release readiness. Status: ACTIVE.
