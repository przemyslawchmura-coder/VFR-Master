...
Research on Demand Wave 8 is COMPLETE for the repository Edge worker
foundation, non-production deployment and live synthetic proof. The Supabase
Edge Function `research-on-demand-worker` completed exactly one synthetic
demand through trusted authorization, durable claim, checkpoint and finish;
duplicate claim returned the terminal state, and exact verification rows were
cleaned up. Secrets remained server-side. No scheduler, provider, UI or
production path exists, and the direction remains demand-driven rather than a
bulk all-motorcycles worker. Provider and UI integration remain deferred.

Research on Demand Wave 9 is COMPLETE for the user-demand intake foundation.
The server-side intake reuses canonical motorcycle context, Wave 1/2 demand
identity, reusable-knowledge lookup and the Wave 5 trusted persistence
boundary. Compatible knowledge is `REUSED`; missing knowledge becomes one
canonical `RESEARCH-REQUIRED` demand; incompatible applicability and unknown
critical context are `CONFLICT-AMBIGUOUS` and fail closed. Repeated demand
does not create duplicate logical work. No worker invocation, API Ninjas or
other provider call, scheduler, UI, production promotion or production/user
mutation was added. Research remains user-demand-driven, not bulk crawling.
