...
test("canonical release metadata is complete and current", () => {
  ...
  assert.equal(metadata.currentVersion, "0.4.0");
  ...
  assert.equal(metadata.releases[0].date, "2026-09-06");
  assert.ok(metadata.releases.some(release => release.version === "0.3.0"));
  assert.ok(metadata.releases.some(release => release.version === "0.2.0"));
