...
test("developer status CLI is deterministic, valid JSON and reports production isolation", () => {
  ...
  const output = JSON.parse(first); assert.ok(output.catalogueTargets > output.productionProfiles); assert.equal(output.productionProfiles, 3); assert.equal(output.structurallyValidResearchDataset, true);
});
