...
test("materialization report is deterministic and production-isolated", () => {
  const first = report(); assert.deepEqual(report(), first); assert.equal(first.validatorResult.valid, true); assert.equal(first.entryCount, 45); assert.equal(first.registryChanged, false); assert.equal(first.runtimeDiscoverable, true); assert.equal(first.evidenceChanged, false); assert.equal(first.serviceCoreCoverageChanged, false); assert.deepEqual(JSON.parse(fs.readFileSync(path.join(__dirname, "../research/reports/ducati-monster937-production-profile-materialization.json"), "utf8")), first);
});
