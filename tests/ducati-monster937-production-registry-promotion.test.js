...
test("normal loader and resolver discover all six Ducati entries losslessly", async () => { const result = await report.buildReport(); assert.equal(result.loaderResult.status, "loaded"); assert.equal(result.loaderResult.profileId, "ducati.monster937.2021"); assert.equal(result.loaderResult.validation.valid, true); assert.equal(result.loaderResult.discovery.status, "found"); assert.deepEqual(result.entryIds, ["ignition.spark-plug.standard", "lubrication.engine-oil.viscosity", "lubrication.engine-oil.specification", "electrical.battery.capacity", "electrical.battery.specification", "brakes.fluid.specification"]); assert.deepEqual(Object.fromEntries(Object.entries(result.resolverResult).map(([id, item]) => [id, item.status])), Object.fromEntries(result.entryIds.map(id => [id, "resolved"]))); assert.deepEqual(result.resolverResult["electrical.battery.capacity"].value, { type: "quantity", amount: 6.5, unit: "Ah" }); });

test("hard exclusions and production boundaries remain explicit", async () => { const result = await report.buildReport(); assert.equal(result.coolingAbsent, true); assert.equal(result.pendingDucatiAbsent, true); assert.equal(result.bmwUnchanged, true); assert.equal(result.vfrUnchanged, true); assert.equal(result.citationsDocumentsUnchanged, true); assert.equal(result.evidenceChanged, false); assert.equal(result.serviceCoreCoverageChanged, false); assert.equal(result.runtimeDiscoverable, true); assert.equal(profile.entries.length, 6); assert.equal(Object.keys(source.documents).length, 1); assert.equal(Object.keys(source.citations).length, 6); });
